...
from transactions.constants import DEPOSIT, WITHDRAWAL,LOAN, LOAN_PAID,TRANSFER_MONEY,TRANSFER, RECEIVED
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        amount = form.cleaned_data.get('amount')
        if not self.request.user.account.is_approved: 
            messages.error(
                self.request,
                'Bank is bankrupt. Withdrawal cannot approve.'
            )
...
class PaisaTransferView(View):
    ...
    def get(self,request):
        form = PoisaTransferForm
        return render(request,self.template_name, {'form': form})
    
    def post(self,request):
        form = PoisaTransferForm(request.POST)
        ...
        if form.is_valid():
            ...
            try:
                to_user = UserBankAccount.objects.get(account_no = Account_ID)
                ...
                if current_user.balance>amount:
                    ...
                    messages.error(request,"Insuffucuent Balance")
